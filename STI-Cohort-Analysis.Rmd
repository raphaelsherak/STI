...
```{r PID Dx}
df.reg <- df %>% select(Filled_Rx, PID, AGE, GENDER, Insurance_3, Race_Eth) %>% set_variable_labels(AGE = "Age", GENDER = "Sex", Insurance_3 = "Insurance", Race_Eth = "Race/Ethnicity")
...
uv.tbl %>% as_gt() %>% tab_header("Univariate ORs")

mv.tbl <- glm(Filled_Rx ~ PID + AGE + GENDER + Insurance_3 + Race_Eth, family = "binomial", data = df.reg) %>% tbl_regression(exponentiate = T)
mv.tbl %>% as_gt() %>%  tab_header("Adusted ORs") 
tbl_merge(list(uv.tbl, mv.tbl), tab_spanner = c("**Univariate ORs**","**Adusted ORs**"))
```
